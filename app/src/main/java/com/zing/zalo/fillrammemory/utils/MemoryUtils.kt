    ...
    */
    ...
    companion object{
        ...
        fun formatToString(byteValue: Long): String {
            ...
            val mbValue = byteValue.div(KBToB).div(MBToKB).toDouble()
            val gbValue = byteValue.div(KBToB.toDouble()).div(GBToKB.toDouble())
        ...
        fun getRemovableValue(): ArrayList<Int>{
            var allocatedSize = MemoryService.mAllocationSize
            val result = ArrayList<Int>()
            do{
                result.add(allocatedSize.toInt())
                allocatedSize /= 2
            } while(allocatedSize > convertMBToBytes(100))
            return if (result.size > 1) result.reversed() as ArrayList<Int>
            else result
        }
    }
}